/* Generates app/data/procedures.generated.ts from scripts/dag-data.json.
 *
 ...
 *
 * Run: node scripts/build-procedures.mjs
 */
...
/* Section 5 "Inputs required per step" - see scripts/extract-inputs.mjs. */
const INPUTS = fileURLToPath(new URL("./inputs-data.json", import.meta.url));
const OUT = fileURLToPath(new URL("../app/data/procedures.generated.ts", import.meta.url));
 ...
 * Source: scripts/dag-data.json + scripts/inputs-data.json (parsed from Docs/Procedures/*.docx)
 * Regenerate: node scripts/build-procedures.mjs
 */
...
export type ProcedureStep = {
   ...
   *  "For physical payment" are kept; lib/requirements.ts interprets them. */
  inputs: string[];
};
