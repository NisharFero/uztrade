...
import { extractShipmentFacts, instantiateWorkflow, type ShipmentFacts } from "../domain/workflow";
...
export async function completeWorkflowWorkItem(
  ...
  return runOrchestrator(repository, item.runId);
}

export async function startWorkflowForCase(caseId: string, procedure: Procedure, query: string, understoodFacts?: ShipmentFacts) {
  const [{ createD1WorkflowRepository }, { getDb }, { eq }, { cases }] = await Promise.all([
    import("./d1-workflow-repository"),
    import("../../db"),
    import("drizzle-orm"),
    import("../../db/schema"),
  ]);
  ...
  await repository.createRun(
    ...
    understoodFacts ?? extractShipmentFacts(query),
  );
  ...
  const { syncCaseBlockProgress } = await import("./legacy-progress");
...
export async function completePersistedWorkflowWorkItem(
  ...
  const projection = await completeWorkflowWorkItem(createD1WorkflowRepository(), workItemId, result, completedBy);
  await syncCaseBlockProgress(projection);
  return projection;
}
